...
| `POST` | `/api/sesiones` | Validar credenciales y obtener los datos del usuario |
| `GET` | `/api/usuarios` | Listar usuarios activos de la empresa del usuario |
...
El backend no usa sesión HTTP. Las operaciones sobre usuarios, procesos y
actividades reciben el parámetro `usuarioId` del usuario que las realiza (por
ejemplo, `GET /api/usuarios?usuarioId=1`). `POST /api/sesiones` valida las
credenciales y devuelve los datos del usuario, incluido su `usuarioId`, sin crear
ninguna sesión. La autenticación se agregará en la entrega final.

Los errores se entregan como respuestas JSON compatibles con `ProblemDetail`.
